...
if __name__ == "__main__":
    ...
    if os.path.exists(args.input):
        config.update(toml.load(args.input))

    flag = args.flag
    flag = 'reaxff_system1'
    if os.path.exists(f"{flag}/run.log"): os.remove(f"{flag}/run.log")        
    logger.add(f"{flag}/run.log", rotation="10 MB", level="INFO")
    logger = logger.bind(name="Non-Equilibrium Partitioned Region Dynamics (ReaxFF/MatterSim):MM")

    # partitioned calculators
    # reax_calc0 = ReaxFFCalculator_LAMMPS(ff_file=f'{flag}/reaxff.ff', tmp_dir=f'{flag}/tmp_reax1', restrain_bond_topo=f'{flag}/oplsaa2_react.data')
    ...
    neff_calc = NeFFCalculator(calc=opls_calc1, neff_file=f'{flag}/neff.neff', work_record_file=f'{flag}/neff.work', bond_record_file=f'{flag}/neff.bond', device=args.device)

    with open(f'{flag}/oplsaa2.data', 'r') as f:
    ...
    print('for statistics, here brute force reset H-atoms masses to a larger one! x 4.0')
    ...
    print(f'test atom pbc: {atoms.get_pbc()}')
