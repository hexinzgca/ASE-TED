...
def read_lammps_dump(filename, index=':'):
    traj = read(filename, format='lammps-dump-text', index=index)
...
if __name__ == "__main__":
    ...
    for fn in [f'{flag}/traj_ne_npt.xyz', f'{flag}/traj_ne_npt.traj']:
        if os.path.exists(fn): os.remove(fn)

    for icycle in range(ncycle):
        no_neq = True if icycle >= ncycle//2 else False

        # load atoms
        if os.path.exists(f'{flag}/final.dump'):
            initial_atoms = read_lammps_dump(f'{flag}/final.dump')
        ...
        # NeFF only RUN
        neffonly_calc.no_neq = no_neq
        integrator.run(interval//4)
        ...
        with open(f'{flag}/run.in', 'w') as f:
            f.write(lammps_input(filename_in='initial.dump', filename_out='final.dump', 
                step=interval))
            f.close()
        try:
            # 执行命令并获取返回码
            ...
            # 检查是否运行成功
            # if ret_code == 0:
            #     print(f"✅ LAMMPS在 {flag} 运行成功，log文件已生成")
            # else:
            #     print(f"❌ LAMMPS运行失败，返回码：{ret_code}，请查看 {flag}/log.lammps")
    ...
    write_frame('traj_ne_npt.xyz', atoms) 

    # final run npt:
    with open(f'{flag}/run.in', 'w') as f:
        f.write(lammps_input(filename_in='initial.dump', filename_out='final.dump', 
            step=config["global"]["steps"]))
        f.close()
    try:
        # 执行命令并获取返回码
        ret_code = os.system(cmd)
